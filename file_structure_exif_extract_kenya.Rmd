...
# exif_orig<-gsub("working_data", "raw_data", exif_out)
# saveRDS(exif_orig, "orig_list_files_RDS_kenya.rds")

exif_out<-readRDS("orig_list_files_RDS_kenya.rds")
```
...
img_nos<-dirname(as.character(exif_out$filepath))
...
```{r, eval = FALSE}
write.csv(exif_out, "kenya_rename_image_file.csv", row.names = FALSE)
```
...
```{r}
exif_out<-read.csv("kenya_rename_image_file.csv", stringsAsFactors = FALSE)
```
...
```{r, eval = FALSE}
saveRDS(exif_out, "kenya_original_filepaths_working_filepaths_2019_10_18.csv.RDS")
write.csv(exif_out, "kenya_original_filepaths_working_filepaths_2019_10_18.csv")
...
sys.stdout = open(os.path.join('D:/Fiona/Biome_Health_Project/exif_output/MN10.txt'), "w")
